...
class Ion_auth
{
      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				$data = array(
					...
					$message = $this->load->view('forgotten_email_temp', $data, true);
					$this->email->clear();
					$this->email->from('babyhealthcare2016@gmail.com',"Baby Healt Care");
					$this->email->to($user->{$this->config->item('identity', 'ion_auth')});
					$this->email->subject('Account Recovery Baby Healt Care');
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		public function is_doctor($ass_num){
				return $this->ion_auth_model->is_doctor($ass_num);
		}
