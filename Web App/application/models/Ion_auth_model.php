...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	public function remember_user($id)
	{
		...
		{
			// if the user_expire is set to zero we'll set the expiration one month from now.
			if($this->config->item('user_expire', 'ion_auth') === 0)
			{
				$expire = (60*60*24*365/12);
			}
			// otherwise use what is set
			else
			{
	...
	**/
	...
	protected function _prepare_ip($ip_address) {
		...
		public function is_doctor($ass_num){
			$q = $this->db->select('*')->from('associational_numbers')->where('number',$ass_num)->get()->row();
